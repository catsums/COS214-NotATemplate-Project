/** @file SignalBus.h
*	@brief Deals with signals that takes in correspondings signal names and subscribe to a handler.
*
...
public:
	...
	~SignalBus(); /**<Destructor*/
	/**
	*	@param string name
	*/
	virtual void emit(string n); /**<emits the signal based on the name. Will create a signalevent and emit it to the corresponding listeners to that event name*/
	/** subscribes a listener/handler to a signal name
	*	@param string name of signal
	*	@param signal handler listener 
	*/
...
protected:
	...
	map<string, vector<SignalHandler*>*> signals; 	/**<a list of signals which listed as a signal name corresponding to a list of listeners*/
	/**
	*	@param string name
	*	@param signal handler event
	*/
	virtual void sendEvent(string n, SignalEvent* event); /**<sends a specific signalevent to every listener to that event name*/
	
	/**
	*	@param string name 
	*/
	virtual SignalEvent* createSignalEvent(string n); /**<creates a signalevent based on the signal name, which by default is the SignalDefaultEvent, but this can be extended*/
};
